...
def createSubNumDict(ranges=[(101, 200), (201, 300), (901, 1000)], key_code_length=20):
    sub_key_dict = {}
    for i in ranges:
...
if not os.path.exists('./mapping_key_to_subId.js'):
    ...
    with open('mapping_key_to_subId.csv', 'w', newline='') as file:
        ...
        for key, val in sub_key_dict.items():
            writer.writerow([val, 'https://experiments.schonberglab.org/static/rani/Space_Gold_App/install.html?subId=' + key, key])
    ...
    print('STOPPING! *** The file already exists ***')
